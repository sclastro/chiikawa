...
不限 `.png`。若手上是 `.jpg`、`.webp` 等，毋須轉檔，只需同步修改 `data/characters.js`（海報則為 `data/stories.js`）中該項的 `image`／`poster` 欄位副檔名。
...
| 用途 | 建議尺寸 | 長寬比 | 備註 |
|---|---|---|---|
| 角色圖 | 600 × 600 px 或以上 | 1:1 正方形 | 以 contain 完整顯示，不會裁走主體；橫向或直向構圖會在框內留白。 |
| 電影海報 | 600 × 900 px 或以上 | 2:3 直度 | 以 cover 填滿，滿版構圖最佳。 |

圖片區底色統一為白色，因此**白底圖效果最好**，會與卡片無縫融合；有色或場景背景會顯示為一塊色塊。建議檔案大小 250 KB 以內。
...
| 檔案路徑 | 用途 | 建議尺寸 | 現況 |
|---|---|---|---|
| `images/stories/movie-poster.jpg` | 劇場版《映画ちいかわ 人魚の島のひみつ》海報 | 600 × 900 px（2:3） | 已有圖片 |
